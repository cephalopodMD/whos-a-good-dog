...
class DollarSign extends Sprite {
  constructor(x, y) {
    ...
    this.setScale(.2, .2)
    var dollarTween = new Tween(this),
        final = new Vec2(16, 16);
    final.transform(Game.getInstance().matrix.inverse())
    dollarTween.animate(TweenableParams.X, x, final.x, 500);
    dollarTween.animate(TweenableParams.Y, y, final.y, 500);
    dollarTween.animate(TweenableParams.ALPHA, 1, 0.5, 500);
